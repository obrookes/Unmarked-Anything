...
TARGET_FPS=1.0,SAM3_MODE=track,DEVICE=auto,USE_HALF=1,OVERWRITE=0,MAX_VIDEOS=,DA3_BATCH_SIZE=0,\
...
12. `da3_batch_size` (optional, default `0` = auto)
...
- `da3_batch_size=0` enables automatic sizing by GPU memory; current defaults pick `24` on >=100GB GPUs (for example GH200), `12` on >=48GB, `8` on >=24GB, otherwise `4`.
